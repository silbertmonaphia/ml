...
class CoTrainingClassifier(object):
    ...
    def fit(self, X1, X2, y):
        ...
        # we randomize here, and then just take from the back so we don't have
        # to sample every time
        random.shuffle(U)
        ...
        # loop until we have assigned labels to everything in U or we hit our
        ...
        while it != self.k_ and U:
            ...
            for i, (y1_i, y2_i) in enumerate(zip(y1, y2)):
                ...
                # with each inner iteration.  We want to add 2p + 2n samples
                # per outer iteration, but classifiers must agree
            ...
            # TODO: Handle the case where the classifiers fail to agree on any
            # of the samples (i.e. both n and p are empty)
    ...
    def predict(self, X1, X2):
        ...
        proba_supported = self.supports_proba(
            self.clf1_, X1[0]) and self.supports_proba(self.clf2_, X2[0])

        # fill y_pred with -1 so we can identify the samples in which the
        # classifiers failed to agree
        ...
        for i, (y1_i, y2_i) in enumerate(zip(y1, y2)):
            if y1_i == y2_i:
                ...
                # the classifiers disagree and don't support probability, so we
                # guess
